...
******************************************************************************/
int compress_yuyv_to_jpeg(Cam *cam, unsigned char *buffer, int size, int quality)
{
    ...
    line_buffer = calloc(cam->width * 3, 1);
    yuyv = cam->framebuffer;
    ...
    cinfo.image_width = cam->width;
    cinfo.image_height = cam->height;
    ...
    z = 0;
    while(cinfo.next_scanline < cam->height) {
        int x;
